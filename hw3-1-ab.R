...
Quandl.api_key("qU3H-wnfNjwj91-1an53")
...
all.returns.tmp <- diff(log(etf.prices))["20141001/20181001"]
...
#all.returns.tmp <- diff(log(cbind(etf.prices,commodity.prices)))["2012/"]
all.returns.tmp <- diff(log(commodity.prices))["20141001/20181001"]
all.returns <- na.omit(all.returns.tmp)
